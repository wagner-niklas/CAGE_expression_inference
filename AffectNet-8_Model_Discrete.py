...
MODEL.classifier = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Linear(block_channels, 8, bias=False),
        )
...
for epoch in range(NUM_EPOCHS):
    ...
    if(valid_loss < best_valid_loss):
        ...
        torch.save(MODEL.state_dict(), 'best_model_affectnet_improved8.pt') # Save the best model

# **** Test the model performance for classification ****

# Set the model to evaluation mode
MODEL.load_state_dict(torch.load('best_model_affectnet_improved8.pt'))
MODEL.to(DEVICE)
MODEL.eval()
