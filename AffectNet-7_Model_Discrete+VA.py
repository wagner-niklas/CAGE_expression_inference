...
for epoch in range(NUM_EPOCHS):
    ...
    for images, classes, labels in tqdm(train_loader, desc ="Epoch train_loader progress"):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16):
            ...
            loss = criterion_cls(outputs_cls.cuda(), classes.cuda()) + 10 * criterion_reg(outputs_reg.cuda() , labels.cuda())
    ...
    with torch.no_grad():
        for images, classes, labels in valid_loader:
            ...
            outputs_cls = outputs[:, :7]
            outputs_reg = outputs[:, 7:]
            loss = criterion_cls_val(outputs_cls.cuda(), classes.cuda()) + 10 * criterion_reg(outputs_reg.cuda() , labels.cuda())
    ...
    if(valid_loss < best_valid_loss):
        ...
        torch.save(MODEL.state_dict(), 'best_model_affectnet_improved7VA2.pt') # Save the best model

# **** Test the model performance for classification ****

# Set the model to evaluation mode
MODEL.load_state_dict(torch.load('best_model_affectnet_improved7VA2.pt'))
MODEL.to(DEVICE)
MODEL.eval()
